...
- (id)initWithCalendarFrame:(CGRect)rect selectedDate:(NSDate *)date {
    if ((self = [self initWithSelectedDate:date])) {
        calendarFrame = rect;
    }
    return self;
}
...
- (void)loadView
{
    ...
    KalView *kalView;
    if (calendarFrame.size.width != 0) {
        kalView = [[[KalView alloc] initWithFrame:calendarFrame delegate:self logic:logic] autorelease];
    }
    else {
